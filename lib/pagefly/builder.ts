 ...
 */
/**
 * The thumbnail strip. `count` is how many tiles are visible at once.
 *
 * It used to be `void count` — the argument was taken and dropped, so every
 * gallery got PageFly's default of five however many the mockup drew. It is a
 * per-breakpoint field on MediaList2 (`fields.md`), so it is written as one.
 */
export function MEDIA_LIST(count: number, styleData: StyleData, itemStyle: StyleData) {
  const show = Math.max(2, Math.round(count) || 5);
  return node(
    "MediaList2",
    /* `slidesToShow` only. The reference writes nothing else here — not even
       `navStyle`, whose documented default is already `none` — and a key the
       editor's own export omits is a key worth omitting. */
...
/* ==========================================================================
   ...
   bordered plate on the left of the photograph and PageFly's stock dark circle
   on the right — in the same gallery, two arrows that do not match.
   ...
   reintroduce it, and a key that names one selector already passes through
   untouched.
   ========================================================================== */
