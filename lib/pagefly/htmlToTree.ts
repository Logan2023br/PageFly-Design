...
export const ASK = [
  ...
  "",
  "  `cardStyle` carries the look, part by part: `card` — THE TILE ITSELF —",
  "  then `cardHover`, `imageHover`, `image`, `title`, `price`, `compareAt`,",
  "  `atc`, `atcHover` (the button's own `:hover` rule), `badge`, `note`, and",
  "  — only when `cardArrow` is true — `nav` for the arrow buttons and",
  "  `navMark` for the shape drawn inside them, written as a `clipPath`",
  ...
  "",
  "  START WITH `card`, AND MOST MOCKUPS SAY MORE ABOUT IT THAN ANY OTHER PART.",
  "  Read the rule on the tile in the markup — the element that wraps the",
  "  photograph and the meta together — and carry its background, border,",
  "  border-radius, padding, box-shadow and transition across. Include",
  "  `overflow` where the rule has it: a photograph inside a rounded card still",
  "  squares off its own top corners without it, which is the difference",
  "  between a finished grid and a nearly-finished one.",
  "",
  "  `cardHover` is what the TILE does when it is pointed at, and `imageHover`",
  "  is what its photograph does — two rules, because that is how the mockup",
  "  writes them:",
  "",
  '    .tile{transition:transform .4s ease, box-shadow .4s ease}',
  '    .tile:hover{transform:translateY(-6px);box-shadow:0 0 40px rgba(0,0,0,.3)}',
  '    .tile:hover img{transform:scale(1.06)}',
  '    → "cardStyle": { "card": {"transition":"transform .4s ease, box-shadow .4s ease"},',
  '                     "cardHover": {"transform":"translateY(-6px)",',
  '                                   "boxShadow":"0 0 40px rgba(0,0,0,.3)"},',
  '                     "imageHover": {"transform":"scale(1.06)"} }',
  "",
  "  `transform` is allowed in these two and nowhere else, for the same reason",
  "  a hover may turn: the card returns to where it started. The transition",
  "  belongs in `card`, with the duration and curve the mockup wrote — a hover",
  "  with no transition snaps, and inventing a duration is deciding how fast",
  "  somebody else's card moves.",
  "",
  "  AND THE BADGE IS INSET BY ITS OWN MARGIN. `badgeCorner` says which corner",
  "  it sits in; how far in from that corner is a `margin` in `badge`. A badge",
  "  the mockup insets 12px from the edge, written without one, arrives flush",
  "  against it.",
  "",
