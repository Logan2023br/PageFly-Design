...
function systemPrompt(ask: SpecAsk): string {
  ...
  return [
    ...
    `card draws a struck-through was-price beside the price. A star rating or`,
    `a review count on the card has no field: those come from a reviews app`,
    `this page cannot read, and inventing one puts a number on the merchant's`,
    `products that nobody measured.`,
    ``,
