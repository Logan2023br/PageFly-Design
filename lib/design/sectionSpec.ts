...
function systemPrompt(ask: SpecAsk): string {
  return [
    ...
    `THE BUY BOX. Where a band holds a "product", design what sits with it —`,
    `this is the section a merchant judges the whole page by, and a stack of`,
    `grey one-liners is what a template looks like. Its rows go in "extras",`,
    `up to ten blocks, and nesting is free: a bordered card is one block.`,
    ``,
