...
function systemPrompt(ask: SpecAsk): string {
  ...
  return [
    ...
    ``,
    `A "product" with "gallery":true carries the gallery's own controls, and`,
    `they exist only if the design gives them one: "mediaStyle" with "nav" is`,
    `an arrow, with "dot"/"dotActive" is pagination — write neither and the`,
    `gallery has neither, which is right for a still photograph with crops`,
    `under it. "mediaControls" is where they sit ("over" the frame, the`,
    `platform's own, or "below" it in a strip of their own); "mediaArrow" is`,
    `the shape inside them ("chevron", the platform's own, or "arrow" for a`,
    `long arrow with a shaft); "mediaThumbs" is how many thumbnails the strip`,
    `shows at once — count them, and leave it out when they are not countable.`,
    `A "product" carries "swatchStyle" for its variant controls: "chip" and`,
    `"chipSelected" are the colour dots, "tile" and "tileSelected" the size`,
    `boxes, "label" the option name, "dropdown" the select. A mockup with`,
    `square size boxes and round colour dots states both — they are different`,
    `shapes and neither is guessable from the other.`,
    ``,
    `A "tabs" carries "tabStyle": "bar" is the strip, "label" and`,
    `"labelActive" the tab words, "panel" the content beneath.`,
    ``,
    `An "anim" carries the motion's own numbers alongside its name: "ms" is the`,
    `duration, "delayMs" the delay, "distance" how far it travels in pixels,`,
    `"easing" the timing function. State them when the design has a timing in`,
    `mind; left out, the builder uses 28px over .7s. "hoverCss" is the :hover`,
    `state itself, for a hover none of the six names covers — stated, the`,
    `element takes a rule of its own instead of the platform's canned one.`,
    ``,
    `An "accordion" carries "accordionStyle": "row" is the clickable question,`,
    `"body" the answer, "icon" the +/- mark.`,
    ``,
    `"mediaHover" is what the main photograph does under the cursor, and the`,
    `default is nothing: write "magnifier" only for a page whose photography is`,
    `forensic enough that a shopper will want to zoom it.`,
    ``,
    `A "beforeAfter" takes "beforeLabel" and "afterLabel" — the two captions`,
    `the mockup paints on the photograph, like "6PM" and "9PM". They are drawn`,
    `now, so write them ONLY when the mockup draws them: left out, the slider`,
    `carries no captions, which is what a mockup without any looks like.`,
    `"compareLabelAt" is where they hang: "corner" pins them to the frame's`,
    `bottom corners, "handle" hangs them on the drag bar so they travel with`,
    `it. READ THE MARKUP — a caption that is a child of the handle is`,
    `"handle"; one that is a child of the frame is "corner".`,
    `An "accordion"'s "accordionStyle" has five parts: "row" is the shut row,`,
    `"rowOpen" the question that is OPEN — the accent there is the only visual`,
    `answer to which one a reader is inside — "rowHover" the row under the`,
    `cursor, "body" the answer, "icon" the mark. Put NO padding in "body": that`,
    `panel stays in the flow when the row is shut, so padding there is room`,
    `every closed row keeps for an answer nobody can see.`,
    ``,
    `An "accordion"'s "accordionStyle.icon" is PageFly's own plus and minus and`,
    `takes SIZE AND COLOUR ONLY. A design drawing its toggle as two hairlines`,
    `in ::before/::after — one rotated, straightening when the row opens — has`,
    `drawn exactly that plus/minus, so give the size and the colour and nothing`,
    `else. Never copy the bars' "background" onto it: on PageFly's icon that`,
    `same declaration is a filled square the width of the icon, and a FAQ came`,
    `back as a column of black squares. A comparison's grip has the same habit`,
    `— a rotated square and a ring drawn as ::before/::after on the handle`,
    `itself, no element and no character anywhere. Those two rules are "mark"`,
    `and "markTwo", which land on the same two pseudo-elements.`,
    ``,
    `Two narrow-width decisions belong to the design too: "galleryPhone" is`,
    `whether a product gallery's thumbnail strip survives on a phone (true`,
    `unless the design really hides it), and "tabBar" is "wrap" or "scroll"`,
    `for a tab bar that runs out of room.`,
    ``,
    `"compareStyle" carries their look. "bar" is the LINE between the two`,
    `photographs: PageFly's own is four pixels wide in a colour you did not`,
    `choose, so a design drawing a seam says "width":"1px" with its own`,
    `colour, and one drawing no line says nothing — left out, the seam is`,
    `ZERO rather than four. "label" is one caption chip,`,
    `"knob" is`,
    `the grip in the middle of the bar — PageFly draws a plain round dot and`,
    `mockups draw a square plate, a bordered circle, whatever. "knobGlyph" is`,
    `the character inside the grip, usually the two-headed arrow — but LOOK`,
    `BEFORE REACHING FOR ONE, because a design rarely types that mark. It`,
    `draws it, as one or two svg shapes, and no character honestly stands in`,
    `for an arbitrary drawing. When it is drawn, COPY IT: "mark" and "markTwo"`,
    `are two more parts of "compareStyle", the first shape and the second, and`,
    `they take ordinary css. A path of straight segments is a "clipPath"`,
    `polygon through the same corners — divide each point's x by the viewBox`,
    `width and each y by its height, write them as percentages, and the shape`,
    `fits whatever size it lands at. Give each mark the "width", "height" and`,
    `"background" the drawing is rendered with. Do that arithmetic against the`,
    `design in front of you: every page draws a different mark, so there is no`,
    `polygon to reuse and nothing to copy but the method. A path with a curve`,
    `in it is not a polygon — leave "mark" out and draw it with "custom".`,
    `Never carry the svg in a script: one "<" makes PageFly refuse the whole`,
    `page's custom JavaScript, so the drawing travels as css.`,
    ``,
    `A "productList" repeats ONE card over the shop's real products, so the`,
    `card is described once and never three times. Read what the mockup's card`,
    `holds: "atcLabel" is its button in the mockup's own words ("Quick add") —`,
    `leave it out when the card has none — and "showCompareAt":true when the`,
    `card draws a struck-through was-price beside the price.`,
    `"atcAt" is where that button sits — "meta" under the price, "image"`,
    `pinned inside the photograph — and "atcReveal":"hover" when it is hidden`,
    `until the card is pointed at. "cardRatio" is the photograph's shape as an`,
    `aspect-ratio ("1 / 1.25"); without it every card is square, and a grid of`,
    `tall photographs arrives cropped. "cardArrow" is false unless the card's`,
    `photograph really pages — one image has nothing to page to, which is`,
    `nearly every card. "cardGap" is the grid's own gap in px, "cardGapPhone"`,
    `what it narrows to on a phone, and "columnsTablet"/"columnsPhone" how many`,
    `cards stand side by side at 768-1024 and at 767 and under — both of those`,
    `were decided in the exporter until now. "badge" is the`,
    `chip over the photograph ("New", "Last pieces") with "badgeCorner";`,
    `PageFly has it built in and a badge modelled as its own node is dropped.`,
    `"cardNote" is the line under the price, and it reads the same on every`,
    `product because the card is one template stamped over the catalogue —`,
    `write it only when the design draws one. "cardStyle" carries the look part`,
    `by part: "image", "title", "price", "compareAt", "atc", "atcHover",`,
    `"badge", "note", and "nav"/"navMark" for the arrows when there are any.`,
    `Every one of those was a constant in the exporter, so a card you do not`,
    `describe is a card wearing somebody else's design.`,
    ``,
    ...
    `on it from the editor. "labels" is the unit names under the figures and`,
    `"separator" is the ":" between the columns — they are independent, and a`,
    `mockup usually draws both. Set "separator":false only when the mockup`,
    `really does run the figures together with nothing between them.`,
    `  layout           row, col`,
    ``,
